...
void Driver::run() {
    ...
    // Division is much slower than multiplication. So calculate 1 / dt and store it as a variable
    double one_div_dt = 1 / dt;

    // TODO FOR LOOP: DRIVER 1 - CLOCK ADDED
//    #pragma omp parallel for
    for(t_current = t_start; t_current < t_end; t_current += dt) {
        step = t_current * one_div_dt + 1;
    ...
    //printf("Dri1: Time elapsed is %f seconds\n", time_spent);
